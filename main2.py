"""
Real-time Vehicle + Number Plate Detection + CSV Logging (RTSP Compatible)
...
RTSP_URL = "rtsp://admin:admin@10.185.207.249:1935"  # <-- Change this to your RTSP link
SERIAL_PORT = None
SERIAL_BAUD = 9600
DETECT_VEHICLE_CLASSES = [2, 3, 5, 7]  # car, motorcycle, bus, truck
OCR_LANGS = ['en']
OUTPUT_VIDEO = "processed_output_rtsp.mp4"
...
if not cap.isOpened():
    print("[ERROR] Cannot open RTSP stream. Check URL or network connection.")
    exit()

# Prepare output video (optional)
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out = None
width, height = int(cap.get(3)), int(cap.get(4))
if width > 0 and height > 0:
    out = cv2.VideoWriter(OUTPUT_VIDEO, fourcc, 20.0, (width, height))
    print(f"[INFO] Output video recording enabled → {OUTPUT_VIDEO}")
else:
    print("[WARN] Could not determine frame size. Skipping output recording.")
# ---------------------------------------------------


print("[INFO] Starting real-time RTSP detection... (Press CTRL+C to stop)")

try:
    while True:
        ret, frame = cap.read()
        if not ret:
            print("[WARN] Lost connection or end of stream. Retrying...")
            time.sleep(2)
            continue  # Try reconnecting automatically
